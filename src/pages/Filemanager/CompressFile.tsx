...
//format: selectedFormat.key,
        try {
            ...
            await dispatch(CompressFileAction({data, action:selectedFormat.key})).unwrap()
            await dispatch(getFileOrDirectory(destinationPath)).unwrap()
      ...
      const errorMessage = error instanceof Error ? error : 'An error occurred during compression';
        addToast({ description: errorMessage.toString(), color: 'danger' });

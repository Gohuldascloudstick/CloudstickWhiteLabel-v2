...
const NewF: React.FC<props> = ({ onClose, parentPath, is_dir }) => {
    ...
    const parentFolder: FileManagerItem | null = useAppSelector((state) => state.fileManager.TheFiles);
    ...
    const executeCreation = async () => {
       ...
       const result =     await dispatch(getFileOrDirectory( parentPath)).unwrap();
console.log(result , 'the result of teh grt in new file');

            if(!is_dir){
              const item = result.data.children.find((it: FileManagerItem) => it.name === name && !it.is_dir);
            console.log(item);
          ...
          console.log(error);
            ...
            addToast({ description: error || "Failed to create item", color: 'danger' });

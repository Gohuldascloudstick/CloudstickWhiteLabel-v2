...
const Permissions: React.FC<PermissionsProps> = ({ item, onClose,parentPath }) => {
  ...
  try {
 await dispatch(ChangePermissions({path:parentPath,name:item.name,permissions:newOctalPermission ,permissionstring:newPermissionString})).unwrap()
//  await dispatch (getFileOrDirectory(parentPath))
    addToast({ description: 'Permissions updated successfully', color: 'success' });
    onClose();
  } catch (error) {
 addToast({ description: error instanceof Error ? error.message : String(error), color: 'danger' });
  }

...
const Extract: React.FC<ExtractProps> = ({ item, onClose, parentPath }) => {
    ...
    const fetchBrowsePath = useCallback(async (path: string) => {
        ...
        try {
            const result = await dispatch(getfileforextart({ path })).unwrap();
    ...
    const handleGoBack = () => {
      console.log(browsePath , 'yes the go back is calling');
    ...
    const handleExtract = async () => {
        ...
        try {
            await dispatch(ExtractFile( { path: item.path, name: item.name, new_path: destinationPath  }))
            await dispatch(getFileOrDirectory(destinationPath ))
